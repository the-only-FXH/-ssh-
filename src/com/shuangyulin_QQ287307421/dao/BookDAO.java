...
public class BookDAO {
    ...
    public ArrayList<Book> QueryBookInfo(String barcode,String bookName,BookType bookType,String publishDate,int currentPage) { 
    	Session s = factory.getCurrentSession();
    ...
    @Transactional(propagation=Propagation.NOT_SUPPORTED)
    public ArrayList<Book> QueryBookInfo(String barcode,String bookName,BookType bookType,String publishDate) { 
    	Session s = factory.getCurrentSession();
    	String hql = "From Book book where 1=1";
    	if(!barcode.equals("")) hql = hql + " and book.barcode like '%" + barcode + "%'";
    	if(!bookName.equals("")) hql = hql + " and book.bookName like '%" + bookName + "%'";
    ...
    /*计算总的页数和记录数*/
    @Transactional(propagation=Propagation.NOT_SUPPORTED)
    public void CalculateTotalPageAndRecordNumber(String barcode,String bookName,BookType bookType,String publishDate) {
        Session s = factory.getCurrentSession();
        String hql = "From Book book where 1=1";
        if(!barcode.equals("")) hql = hql + " and book.barcode like '%" + barcode + "%'";
        if(!bookName.equals("")) hql = hql + " and book.bookName like '%" + bookName + "%'";
